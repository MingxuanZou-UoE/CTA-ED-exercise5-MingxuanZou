...
Get the text from Project Gutenberg; token it and create the dfm for LDA model
```{r}
...
```{r}
topsy_lda <- LDA(topsy_dtm, k = 7, control = list(seed = 1234)) #specify 7 topics that we want to search for, and we can also set our seed
```


extract the per-topic-per-word probabilities
```{r}
topsy_topics <- tidy(topsy_lda, matrix = "beta") #tidied up the beta matrix in the dataset -- to get the topic-term probabilities

head(topsy_topics, n = 10) #display top 10 rows of the dataset
```


plots the top terms
```{r}
topsy_top_terms <- topsy_topics %>% #create a new dataset named tposy_top_terms
  group_by(topic) %>%               #group the dataset according to topic
  top_n(10, beta) %>%               #select the top 10 rows based on the 'beta' 
  ungroup() %>%                     #remove the grouping structure
  arrange(topic, -beta)             #arranges the rows first by topic and then by beta in descending order within each topic group

topsy_top_terms %>%
  mutate(term = reorder_within(term, beta, topic)) %>% #create a new variable 'term', which arranges 'term' within each 'topic' group based on the 'beta' column(higher beta, higher position).
  ggplot(aes(beta, term, fill = factor(topic))) +      #create a ggplot object, where 'beta' is x-axis, 'term' is y-axis, and  each 'topic' represented by different fill colors
  geom_col(show.legend = FALSE) +                      #specifies that the plot should use columns to represent the data and 'show.legend = FALSE' is used to hide the legend as we alread set the fill color as 'topic'
  facet_wrap(~ topic, scales = "free", ncol = 4) +     #create multiple plots,each presenting a different topic.
  scale_y_reordered() +                                #allows for reordered y-axis scales.
  theme_tufte(base_family = "Helvetica")               #sets the theme for the plot
```
